...
> 最新發佈：v2.1.9（預編包見 [Releases](https://github.com/chinoyan2002/luci-app-geoguard/releases)）
...
> Latest release: v2.1.9 (prebuilt packages under [Releases](https://github.com/chinoyan2002/luci-app-geoguard/releases))
...
- Cron jobs carry comments (following the LuCI language); action buttons serialized + locked while busy; untouched settings are never overwritten on save (per-group dirty guards); the package only builds sets and never touches firewall rules; one setting binds a set to a rule (step-by-step guide on the page)
