...
(async () => {
  ...
  try {
    ...
    await page.goto('http://192.168.1.1/cgi-bin/luci/admin/network/countryallow',
      { waitUntil: 'networkidle2', timeout: 30000 }).catch(() => {});
    // 直接拼 URL 會掉 session（stok 機制），改走選單點擊
    let hasView = await page.$('#cbi-countryallow');
    if (!hasView) {
      await page.goto('http://192.168.1.1/cgi-bin/luci/', { waitUntil: 'networkidle2', timeout: 30000 });
      ...
      await page.evaluate(() => {
        const a = [...document.querySelectorAll('#topmenu a')].find((x) => x.textContent.trim() === 'Country Allow List');
        if (a) a.click();
      });
      await page.waitForSelector('#cbi-countryallow', { timeout: 25000 });
    }
    await page.waitForSelector('#cbi-countryallow', { timeout: 20000 });
    await new Promise((r) => setTimeout(r, 4000)); // 等 view 非同步 render
    const checks = await page.evaluate(() => {
      const out = {};
      const view = document.querySelector('#cbi-countryallow');
      out.hasView = !!view;
      const tabs = [...document.querySelectorAll('.cbi-tab, .cbi-tab-disabled')].map((t) => t.textContent.trim());
      out.tabs = tabs;
      const btns = [...document.querySelectorAll('#cbi-countryallow button')].map((b) => b.textContent.trim()).filter(Boolean);
      out.buttons = btns;
      const rows = document.querySelectorAll('#cbi-countryallow table tbody tr');
      out.countryRows = rows.length;
      const thead = document.querySelector('#cbi-countryallow thead');
      out.theadSticky = thead ? getComputedStyle(thead).position : 'none';
      const search = document.querySelector('#cbi-countryallow input[placeholder*="搜尋"]');
      out.hasSearch = !!search;
      const sel = document.querySelectorAll('#cbi-countryallow table tbody tr td:first-child input[type="checkbox"]');
      out.rowChecks = sel.length;
      const pre = document.querySelector('#cbi-countryallow pre');
      out.hasPre = !!pre;
      return out;
    });
